...
const EditFilterModal = ({
    ...
    const [curSelectedCodes, setCurSelectedCodes] = useState(structuredClone(selectedCodes))
    ...
    const addCodes = (codesAr) => {
        setPreparedGroup((prevState) => setSelectedByCodes(codesAr, [...prevState], true))
        setCurSelectedCodes((prevState) => [...prevState, ...codesAr])
    }

    const removeCodes = (codesAr) => {
        setPreparedGroup((prevState) => setSelectedByCodes(codesAr, [...prevState], false))
                    ...
                    {preparedGroup.map((item) => (
                        filterName === 'vessels' ? (
                            ...
                            <CheckboxRow
                                key={item.code}
                                title={item.value}
                                selected={curSelectedCodes.includes(item.code)}
                                disabled={
                                    disabledItems
                                        ? disabledItems?.has(item.code)
                                        : (!curSelectedCodes.includes(item.code) && item.selected)
                                }
