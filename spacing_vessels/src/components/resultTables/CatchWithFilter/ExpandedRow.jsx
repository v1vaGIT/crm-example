...
import * as s from '../CatchWithFilter/style.module.scss'
...
const ExpandedRow = ({row}) => {
    ...
    return (
        <>
            <tr>
                <td className={s.firstRowItem} onClick={() => setIsShowChildren(!isShowChildren)}>
                    {row.vessel.name}
                    <svg
                        width='16'
                        height='16'
            ...
            {isShowChildren &&
                row.fishingZones.map((zone) => (
                    <tr key={zone.fishingZone.id} className={s.childrenRow}>
                        <td>{zone.fishingZone.name}</td>
                        {monthListEng.map((month) => (
                            <td key={month}>{zone.months[month].toLocaleString('ru')}</td>
                        ))}
                        <td>{zone.total.toLocaleString('ru')}</td>
                    </tr>
                ))}
        </>
    )
